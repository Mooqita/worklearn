...
s_a[27] = "Federation of Bosnia and Herzegovina|Republika Srpska|Brčko District";
...
s_a[250] = "Vojvodina|Belgrade|Western|Southern Eastern";
...
Template.country_state.onCreated ->
	...
	index = country_arr.indexOf(country)

	this.country_index = new ReactiveVar(index);
	this.country = new ReactiveVar(state);
	this.state = new ReactiveVar(country);

Template.country_state.helpers
	countries: () ->
		res = []
		index = 0
		for country, index in country_arr
			item =
				index: index
				country: country
			res[index] = item
		...
		res.sort (a,b) ->
			return if a.country.toUpperCase() >= b.country.toUpperCase() then 1 else -1

		return res

	states: () ->
		index  = Number(Template.instance().country_index.get()) + 1
		states = s_a[index]
		return states.split('|')
