...
_refresh = (mails)->
	...
	for mail in mails
		para = '<p data-tokens="' + mail + '">' + mail + '</p>'
		html = '<option data-tokens="' + mail + '">' + mail + '</option>'
		$('#user_select').append html
		$('#empty_box').append para

	$('.user_select_class').selectpicker 'refresh'
	$('.empty_box_class').selectpicker 'refresh'

#########################################################
Template.learners.onCreated ()  ->
	self = this
	self.selected = new ReactiveVar([])
	self.all = new ReactiveVar([])
	self.candidates = new ReactiveVar([])
	o_id = FlowRouter.getQueryParam("organization_id")
	l_id = FlowRouter.getQueryParam("user_id")

	self.autorun () ->
		self.subscribe "learner_by_user_id", l_id
